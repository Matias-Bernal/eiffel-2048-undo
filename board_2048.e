...
feature -- Status report
	...
	set_elements (new_elements: ARRAY2 [CELL_2048])
		local
			i: INTEGER
			j: INTEGER
		do
			...
			loop
				...
				until
					j > columns
				loop
					set_cell (i, j, new_elements.item (i, j).value)
					j := j + 1
				end
				i := i + 1
			end
	...
	clone_board_2048: BOARD_2048
		local
			new_board: BOARD_2048
		do
			create new_board.make_empty
			new_board.set_elements (elements)
			Result := new_board
		end
